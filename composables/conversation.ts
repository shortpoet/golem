...
export const useConversations = () => {
  ...
  const knowledgeUsedInConversation = computed(() => {
    ...
    return (
      currentConversation.value.knowledge
        ?.map((knowledgeId) => {
          return knowledgeList.value?.find(
            (knowledge) => knowledge.id === knowledgeId
          );
        })
        .filter((knowledge) => knowledge !== undefined) ||
      ([] as types.KnowledgeItem[])
    );
  });
  ...
  const followupQuestions = useState<Record<string, Array<string>> | null>(
    () => null
  );
  ...
  async function createConversation(
    title: string,
    options?: Partial<types.Conversation>
  ) {
  ...
  async function updateLastAssistantMessage(
  ...
  const updateConversation = async (
    id: string,
    update: Partial<types.Conversation>
  ) => {
    const conversation = await db.table("conversations").get(id);
    if (!conversation) {
      throw new Error("Conversation not found");
  ...
  async function clearErrorMessages() {
    ...
    const newMessages = conversation.messages.filter(
      (message: types.Message) => !message.isError
    );
  ...
  const sendMessage = async (message: string) => {
    ...
    const assistantMessageList = (fromConversation.messages || []).filter(
      (message: ChatMessage) => message.role === "assistant"
    );
    const lastAssistantMessage =
      assistantMessageList[assistantMessageList.length - 1];
    ...
    let messageList: any[] = getMessageChain(
    ...
    if (fromConversation.systemMessage) {
      messageList = [
        {
          role: "system",
          text: fromConversation.systemMessage,
          id: "system-message",
        },
        ...messageList,
      ];
    ...
    const getTokenCount = () =>
      encode(messageList.map((message) => message.content).join("\n\n")).length;
    ...
    if (getTokenCount() > MaxTokensPerModel[modelUsed.value]) {
      ...
      while (
        ...
        lastTokenCount > MaxTokensPerModel[modelUsed.value]
      ) {
        messageList = [
          messageList[0],
    ...
    const upsertAssistantMessage = async (
      messageResponse: types.Message,
      finalUpdate?: boolean
    ) => {
      if (!thisMessage) {
        thisMessage = await getMessageById(
          fromConversation.id,
          messageResponse.id
        );
      }
      ...
      if (thisMessage) {
        ...
        if (finalUpdate) {
          if (isDetaEnabled.value) {
            deta.message.update(
              getUpdatedMessage(messageResponse, fromConversation.id)
            );
          }
        }
    ...
    const { data: assistantMessage, error: messageError } = await handle(
      sendMessage({
      ...
      const errorHandlerMapping = {
        async model_not_found() {
          await addErrorMessage(
            "The model you are using is not available. Please select another model in the settings."
          );
        },
        ...
        async invalid_api_key() {
          await addErrorMessage(
            "Your API key is invalid. Please check your API key in the settings."
          );
        },
  ...
  async function generateConversationTitle(conversationId: string) {
    ...
    const lastMessagesContent = lastMessages.map(
      (message: ChatMessage) => message.text
    );
    const conversationTitle = await complete(lastMessagesContent.join("\n"), {
      systemMessage:
        "You are a very clever machine that can determine a very short title for a conversation. The user sends you the content of a conversation and you only output a very short title for it, really concise. Title:",
      temperature: 0,
  ...
  async function removeMessageFromConversation(
    conversationId: string,
    messageId: string
  ) {
    const conversation = await getConversationById(conversationId);
  ...
  async function clearConversations() {
    ...
    await Promise.all(
      conversationList.value.map((conversation: types.Conversation) =>
        limit(() => deleteConversation(conversation.id))
      )
    );
  ...
  async function getFollowupQuestions(text: string) {
    ...
    followupQuestions.value = {
      ...followupQuestions.value,
      [currentConversationId.value]: (response.data.choices[0].text || "")
        .split("\n")
  ...
  async function updateConversationMessage(
    ...
    const messageIndex = conversation.messages.findIndex(
      (m: types.Message) => m.id === messageId
    );
    if (messageIndex === -1) {
      return;
    }
...
function getMessageChain(
  ...
  const parentMessage = messages.find(
    (m: ChatMessage) => m.id === message.parentMessageId
  );
  if (!parentMessage) {
    return [message];
  }
