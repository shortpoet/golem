...
const { isDetaEnabled } = { isDetaEnabled: false }
...
<template>
  <div v-if="!embedded" absolute top-0 left-0 right-0 b-0 b-b-1 b-gray-1 dark:b-dark-1 b-solid py-3 z-1 backdrop-blur-4
    mx-auto class="bg-white/90 dark:bg-dark-1/80" :class="[
    ...
    <div w-full px-4 flex items-center>
      <div>
        <div flex items-center gap-1>
          ...
          <Transition name="appear-left">
            <div v-if="!isEditingTitle" text-5 mr-1 cursor-pointer hover:scale-120 transition :class="[
          ...
          <input ref="inputElement" v-model="conversationTitle" :readonly="!isEditingTitle" bg-transparent border-none
            outline-none p-0 transition font-bold grow mr-3 truncate text-14px sm:text-18px h-1.75em :class="[
              isEditingTitle
                ? 'text-gray-900 dark:text-gray-1 select-all'
                : 'text-gray-700 dark:text-gray-3 select-none cursor-pointer',
            ]" @blur="isEditingTitle = false" @input="onInput" @dblclick.stop="onTabDoubleClick">
        </div>
      </div>
      <!-- TODO: implement sharing -->
      <UButton v-if="!embedded && isDetaEnabled" ml-auto icon="i-tabler-share" @click="onShare">
        Share
      </UButton>
    </div>
